    ...
    if (strpos($processedCell, "yes") === false) {
        ...
        foreach ($headers as $index => $header) {
            ...
            if (strpos($headerValue, "email address") !== false) {
                ...
                $columnHIndex = $spreadsheetUpdate->findHeaderIndex($headers, 'Please select what this request is for');
                $columnHColumnLetter = $spreadsheetUpdate->util->numberToColumnName($columnHIndex + 1);
